...
[Kaggle ultrasound-nerve-segmentation challenge](https://www.kaggle.com/c/ultrasound-nerve-segmentation) is one of the most high profile challenges ever hosted on Kaggle. We have unleashed the power of [torchnet](https://github.com/torchnet/torchnet) for tackling the challenge and received some remarkable results. The challenge in itself acts as a learning experience for segmentation problems. Figure below is an example of the image and the mask to predict.
...
We are using a Modified [U-Net](http://arxiv.org/abs/1505.04597) with [Kaiming-He](https://arxiv.org/abs/1502.01852) initialization. The structure of U-Net generated using nngraph can be found [here](/assets/images/U-Net.svg)
...
For our model to generalize as it converges, providing a shuffled dataset on every epoch is an important strategy. So we enclose, our data in [list dataset](https://github.com/torchnet/torchnet#tntlistdatasetself-list-load-path), which is simply loading data in list and then wrap it with [shuffle dataset](https://github.com/torchnet/torchnet#tntshuffledatasetself-dataset-size-replacement), which ensures that whenever you query the dataset, you get the image chosen at random from the dataset without replacement.
...
Once we have loaded the complete dataset, the next part is to create an iterator. We call the iterator in every epoch, and it returns the batch over which training must be done. Before a batch is put for training, we must ensure that transformations take place and then batch is formed of the given size. Using shuffled dataset ensures that we get new order of data every epoch and thus batches are non-uniform across the epochs. [Batch dataset](https://github.com/torchnet/torchnet#tntbatchdatasetself-dataset-batchsize-perm-merge-policy) ensures that batches are formed from underlying images.
