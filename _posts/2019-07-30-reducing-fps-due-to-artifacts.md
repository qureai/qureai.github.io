...
Medical Imaging is often accompanied by acquistion artifacts which can be subject related or hardware related. These artifacts make confident diagnostic evaluation difficult in two ways:
...
<br/>
<small class="caption">Fig 2. Artifact due to subject motion, mimicking a subdural hemorrhage in a Head CT</small>
</center>
...
* Hardware artifact- See fig 5. This artifact is caused due to  air bubbles in the cooling system. There are subtle irregular dark bands in scan, that can be misidentifed as cerebral edema.

<center>
<img width='80%' src="/assets/images/artifact/artifact-edema.png" alt="hardware artifact edema"/>
<br/>
<small class="caption">Fig 5. Another hardware related artifact, mimicking cerebral edema marked by yellow arrows.</small>
</center>


 Hence, CT images often have severe artifacts which cause mimics that increase the false positive (FPs) predictions of AI models.
Here we are investigating motion artifacts that look like SDH, in Head CT scans. These artifacts result in increase in false positive (FPs) predictions of subdural hemorrhage models. We confirmed this by quantitatively analyzing the FPs of our AI model deployed at an urban outpatient center. The FP rates were much higher for this data when compared to our internal test dataset. 
One reason why these artifacts result in false positives is due to the lack of variety of artifacts in the training set used. Its practically difficult to include all varieties of artifacts in the training set.
...
<br/>
<small class="caption">Fig 7. Model identifies an artifact slice as SDH because of similarity in appearance and location</small>
</center>
...
* Discounting slices with artifact when calculating the probability of bleed in a scan. This is very similar to how radiologists read head CT scans.


### Method 1. Artifact as an augmentation using Cycle GANS 

We reasoned that the artifacts were misclassified as bleeds because the model has not seen many artifact scans while training. 
The number of images containing artifacts is relatively small in our annotated training dataset. But we have access to several unannotated scans containing artifacts acquired from various centers in India. If we could generate artifact ridden versions of all the annotated images in our training dataset, we would be able to effectively augment our training dataset and make the model invariant to artifacts. 
We decided to use a cycle GAN to generate new training data containing artifacts.

Cycle GAN is a generative adversarial network that is used for unpaired image to image translation. 
For example, if we have a set X of images of horses and a set Y of images of zebras, but each horse image in set X does not have a directly comparable zebra image in set Y with the sampe pose, lighting etc. Cycle GAN can be used in this scenario to make horses look like zebras and vice versa. CycleGAN is a Generative Adversarial Network (GAN) that uses two generators and two discrimintators.We call one generator _G_, and have it convert images from the X domain to the Y_ domain. The other generator is called F, and converts images from Y to X.
Each generator has a corresponding discriminator, which attempts to tell apart its synthesized images from real ones.

The loss function consists of an adversarial loss and a cycle consistency loss. Adversarial loss ensures that zebra images generated look natural in Y domain and cycle consistenscy loss ensures that the zebra image generated is similar to that of horse. This is done by enforcing F(G(x))=x  and G(F(y)) =y. 

The cycle gan serves our purpose because we have an unpaired image translation problem where X domain has our training set CT images with no artifact and Y domain has artifact ridden CT images. 

<center>
<img width='80%' src="/assets/images/artifact/cycle_gan_illustration_with_loss.png" alt="cycle gan illustration"/>
<br/>
<small class="caption">Fig 8. Cycle GAN illustration</small>
</center>


We curated a X dataset of 5000 images without artifact and Y dataset of 4000 images with artifacts  and used this to train the Cycle GAN.
Unfortunately, the quality of generated images were not very good. See fig 4. 
The generator was unable to capture all the variety in CT dataset, meanwhile introducing artifacts of its own, thus rendering it useless for augmenting the dataset.  Cycle GAN authors state that the performance of the generator when the tranformation involves geometric changes for ex. dog to cag, apples to oranges etc. is worse when compared to tranformation involving color or style changes. Inclusion of artifacts is  a bit more complex than color or style changes because it has to introduce distortions to existing geometry. This could be one of the reasons why the generated images have extra artifacts. 

<center>
<img width='120%' src="/assets/images/artifact/cyclegan_generated_images.png" alt="cycle gan images"/>
<br/>
<small class="caption">Fig 9. Sampling of generated images using Cycle GAN</small>
</center>
...
<br/>
<small class="caption">Fig 10. Method 2 Using a trained artifacts model to discout artifact slices while calculating SDH probability.</small>
</center>
...
<br/>
<small class="caption">Fig 11. Confusion Matrix before and after using artifact model for SDH prediction</small>
</center>


See fig 12. for model predictions on a representative scan.

<center>
<img width='100%' src="/assets/images/artifact/discount_explanation.jpeg" alt="artifact discount explanaation"/>
<br/>
<small class="caption">Fig 12. Model predictions for few representative slices in a scan falsely predicted as positive by original SDH model</small>
</center>
